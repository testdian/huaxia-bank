  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        text: '行业配置页移除「新增行业」「批量导入」及演示环境 DEV 导入说明入口，列表操作列移除「删除」，仅保留标识筛选与「编辑」。主系统基础配置已包含行业配置功能，仅需增加配置标识功能：在既有国标四级行业上维护「人行八大高碳」「我行主要行业」等标识，供候选清单、正式清单与核算流程按标识筛选与展示。',
      ...
      },
      {
        ...
        text: '顶栏新增演示角色切换：总行管理部门、分行负责人、客户经理三类视角可一键切换，菜单与数据范围随角色联动。切换为「客户经理」后，额外展示客户经理下拉（王磊/陈静/刘洋/赵敏/周强/李娜，含所属分行），便于分别登录不同主办客户经理视角填报与验收任务清单、收集填报、审核流转等流程。该切换器仅供 UAT/演示测试使用，正式系统以宿主系统统一认证与授权为准，不提供此演示下拉。',
        route: '#/manager-tasks',
