...
const UPDATE_CHANGELOG = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        text: '排放计算页「排放计算清单」上方新增筛选：项目总投资金额（元）起、项目总投资金额（元）止。查询后下方清单按投资额区间过滤；顶部总归因排放量、DQR/质量评级、已计算笔数及「投融资碳排放强度」全行与分行业统计均随当前筛选结果实时重算。点击「一键提交数据」时，系统按当前筛选清单锁定数据；后续核算结果查询、报告导出与企业碳账户归集仅包含锁定范围内的记录。',
        route: '#/calculation',
        screenshot: `${CHANGELOG_SCREENSHOT_BASE}/calculation-invest-filter.png?v=20260715j`
      },
      {
        menu: '演示环境',
        feature: '角色与客户经理切换',
