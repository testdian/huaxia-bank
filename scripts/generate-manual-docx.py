...
MANUAL_VERSION = 'v0.2'
MANUAL_DATE = '20260527'  # 定稿日期 YYYYMMDD
...
def build():
    ...
    add_title(doc, '华夏银行绿金系统')
    add_title(doc, '投融资碳核算 · 企业碳账户', level=1)
    ...
    add_para(doc, '编制说明：本手册基于绿金系统演示原型编写，涵盖「投融资碳核算」与「企业碳账户」两大模块；两模块数据贯通，操作按钮以【】标注。v0.2 在 v0.1 基础上新增企业碳账户操作说明及模块关联说明。')
    ...
    add_para(doc, '总行绿金部：可发起任务；清单确认与锁定；派发补录、经济法直算；总行发起任务时执行总行终审；排放计算【确认结果】（触发碳账户归集）；报告导出；企业碳账户全行查询与账户状态管理；排放因子库与接口管理。')
    add_para(doc, '分行负责人：可发起本分行业务；辖内补录组织与进度跟踪；总行发起任务时做分行初审，分行发起任务时做分行终审；企业碳账户仅查看辖内（按一级分行过滤）数据。')
    add_para(doc, '客户经理：接收本人名下补录任务；在线填报（五类核算方法择一）；提交数据与提交审核；查看审批进度；无企业碳账户菜单。')

    doc.add_heading('1.6 投融资碳核算与企业碳账户的关联', level=2)
    add_para(doc, '「投融资碳核算」与「企业碳账户」是绿金系统内前后衔接、数据同源的两个模块：前者按核算任务完成年度（或专项）碳核算并产出报告；后者以客户维度（法人 + 贷款号）长期归集已确认的排放结果，支撑查询、分析与账户治理。')
    add_para(doc, '（1）数据主键与建档规则')
    add_para(doc, '投融资碳核算以「核算任务」为主线，清单与计算记录挂在任务下。企业碳账户以「统一社会信用代码 + 贷款号」唯一建档（同一法人、同一贷款号仅一个账户）；首次有符合条件的排放被确认归集时自动开户。')
    add_para(doc, '（2）归集触发时机（关键衔接点）')
    add_para(doc, '在核算任务「排放计算」页，当该任务下计算均已完成且数据齐备时，总行点击【确认结果】。系统将该任务内每条已完成计算（status=done、含归因排放）写入企业碳账户的「排放记录」，并关联对应账户；同时提示「排放记录已归集至企业碳账户」。此后可在侧栏【企业碳账户】查看，无需重复录入。')
    add_para(doc, '（3）纳入碳账户的数据范围')
    add_para(doc, '纳入：正式清单已锁定、计算已完成、且任务已【确认结果】的记录；须具备有效统一社会信用代码与贷款号。')
    add_para(doc, '不纳入：候选/正式清单未锁定、补录填报中或已退回、未完成排放计算、任务未确认结果的数据。')
    add_para(doc, '（4）记录字段与台账一致性')
    add_para(doc, '碳账户「排放明细」列表字段与候选业务清单（信贷台账）列对齐，并追加核算年度、主体/归因排放、碳强度、核算方法、核算完成时间等；便于从客户视角回溯至原核算任务（记录中含 taskId、calcId）。')
    add_para(doc, '（5）权限与视角差异')
    add_para(doc, '总行：全行账户与记录；可对账户执行启用、停用、注销（CA003）。分行：仅本一级分行辖内账户与记录（含辖内各经办行明细）。客户经理：侧栏不展示企业碳账户。')
    add_para(doc, '（6）与报告导出的关系')
    add_para(doc, '任务级监管/管理报告仍在「生成报告」导出；企业碳账户侧重客户维度的历史排放汇总、多维图表与明细导出（【导出 Excel】），二者互补，不互相替代。')
    ...
    sections_hq = [
        ...
        (
            '3.6 碳排放计算与核算结果',
            '汇总主体排放、归因排放与分行业占比；【确认结果】后数据自动归集至企业碳账户，方可导出任务报告。',
            [
                ('进入「排放计算」查看汇总。', 'HQ-20-排放计算', '图 3-20 排放计算'),
                ('核对后点击【确认结果】（系统提示已归集至企业碳账户）。', 'HQ-21-确认结果', '图 3-21 确认结果'),
        ...
        (
            '3.10 企业碳账户',
            '全行客户碳账户列表与详情；数据来源于各任务【确认结果】后的自动归集。总行可管理账户状态（启用/停用/注销）。',
            [
                ('侧栏进入【企业碳账户】；顶栏切换核算年度（仅展示该年度有排放记录的账户）。', 'CA-01-列表总览', '图 3-30 企业碳账户列表'),
                ('使用企业名称/信用代码/贷款号、一级分行、账户状态筛选，点击【查询】或【重置】。', 'CA-02-年度与筛选', '图 3-31 列表筛选'),
                ('列表点击【查看】进入账户详情；对启用账户可【停用】或【注销】，停用后可【启用】（弹窗确认）。', 'CA-03-账户状态管理', '图 3-32 账户状态管理'),
                ('详情页「账户档案」查看法人、贷款号、行业、主办分行及状态变更记录。', 'CA-04-账户档案', '图 3-33 账户档案'),
                ('「排放明细」按年度/行业/经办行等筛选，列与信贷台账对齐；可【导出 Excel】。', 'CA-05-排放明细', '图 3-34 排放明细'),
                ('「多维汇总」查看项目/非项目、行业、年度、贷款类型等图表。', 'CA-06-多维汇总', '图 3-35 多维汇总'),
                ('「趋势分析」查看年度排放与贷款余额碳强度（tCO₂e/万元）趋势。', 'CA-07-趋势分析', '图 3-36 趋势分析'),
            ],
        ),
    ]
    ...
    sections_br = [
        ...
        (
            '4.4 企业碳账户（辖内查看）',
            '分行仅可查看本一级分行辖内账户（含辖内各经办行排放明细），不可变更账户状态。',
            [
                ('顶栏切换「分行负责人」，侧栏【企业碳账户】；页头提示当前辖内视角。', 'CA-08-分行辖内列表', '图 4-9 分行辖内碳账户'),
                ('筛选后【查看】进入详情，操作同总行（无停用/注销按钮）。', 'CA-04-账户档案', '图 4-10 辖内账户详情（档案）'),
            ],
        ),
    ]
    ...
    doc.add_heading('六、端到端演示路径', level=1)
    doc.add_heading('6.1 投融资碳核算主流程（约 10 分钟）', level=2)
    ...
    add_para(doc, '5. 总行：终审 → 排放计算页【确认结果】（排放归集至碳账户）→ 报告【导出 Excel】')
    add_para(doc, '演示前可点击顶栏【重置数据】加载样例任务 T2025001（含已归集碳账户演示数据）。')

    doc.add_heading('6.2 企业碳账户查看路径（约 3 分钟）', level=2)
    add_para(doc, '1. 总行：侧栏【企业碳账户】→ 切换核算年度 → 筛选客户 →【查看】')
    add_para(doc, '2. 详情：「排放明细」核对与台账一致的字段 →【导出 Excel】')
    add_para(doc, '3. 「多维汇总」「趋势分析」查看客户历史结构')
    add_para(doc, '4. 返回列表：可选【停用】/【启用】演示账户治理（CA003）')
    add_para(doc, '5. 切换「分行负责人」：仅见北京分行辖内账户，验证权限隔离')

    doc.add_heading('6.3 两模块串联演示要点', level=2)
    add_para(doc, '若需现场演示「核算 → 归集」闭环：在全新重置数据后走完 6.1 至【确认结果】，再进入企业碳账户，应能看到对应客户新增排放记录；同一法人+贷款号多次核算将归入同一账户。')
