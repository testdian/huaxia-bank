#!/usr/bin/env python3
"""生成投融资碳核算模块需求规格说明书 v0.6
v0.6 新增：完整数据流转图、月均余额计算细节、候选→正式字段映射、
数据采集客户归集合并规则、各核算方法采集字段全集、归因排放公式细化、
方法模板与参数字段库联动说明。"""
...
OUT      = ROOT / "docs" / "华夏银行绿金系统-投融资碳核算模块_需求规格说明书-v0.6.docx"
...
def feat(doc, no, name, desc, input_rows, flow_lines, check_lines,
    ...
    if check_lines:
        fp(doc, "校验：")
        for c in check_lines: lp(doc, c)
    fp(doc, "处理流程：")
    for fl in flow_lines: lp(doc, fl)
...
def build_doc():
    ...
    for line in ["起草人：Cursor", "起草日期：2026.06.26", "文档版本号：v0.6"]:
    ...
    add_plain_table(doc, ["对应需求编号", "版本", "起草/修订人", "说明"], [
        ["CR21093-A", "v0.6", "Cursor",
         "投融资碳核算模块需求规格说明书，v0.6补充完整数据流转与计算逻辑"],
    ])
    ...
    np(doc, "v0.6相比v0.5的主要增补：完整端到端数据流转图；月均贷款余额/平均资产总额的逐步计算细节；候选清单至正式清单的字段继承规则；数据采集客户归集合并规则（含伪代码）；各核算方法在收集填报中所需字段的完整清单；归因排放的多档计算公式；方法模板→参数字段库→收集填报的联动机制；数据采集后拆分到逐笔归因的算法说明。")
    ...
    np(doc, "")
    np(doc, "— 文档结束 —")
    np(doc, "版本 v0.6 · 2026.06.26 · 起草：Cursor · 待评审")
